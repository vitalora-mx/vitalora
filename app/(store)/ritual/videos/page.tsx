    ...
    <main>
      ...
      <div style={{ background: '#0E0E0E', minHeight: '600px' }}>
        <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '48px 40px 80px' }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-italiana), serif', fontSize: '42px', color: '#F5F0E8', margin: '20px 0 8px' }}>Todos los videos</h1>
          <p style={{ fontSize: '14px', color: 'rgba(245,240,232,0.6)', marginBottom: '48px' }}>Explora por tema</p>

          {loading ? (
            <p style={{ textAlign: 'center', color: 'rgba(245,240,232,0.6)', padding: '60px' }}>Cargando videos...</p>
          ) : videos.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '80px', color: 'rgba(245,240,232,0.6)' }}>
            ...
            grupos.map(grupo => (
              <div key={grupo.tema} style={{ marginBottom: '48px' }}>
                <h2 style={{ fontFamily: 'var(--font-cormorant), serif', fontSize: '24px', color: '#F5F0E8', marginBottom: '20px' }}>{grupo.tema}</h2>
                  ...
                  {grupo.videos.map(video => (
                    <Link key={video.id} href={`/ritual/${video.slug}`} style={{ textDecoration: 'none', flex: '0 0 300px' }}>
                      <div style={{ cursor: 'pointer', borderRadius: '6px', overflow: 'hidden', transition: 'transform 0.3s' }}
                        ...
                        <h3 style={{ fontFamily: 'var(--font-cormorant), serif', fontSize: '16px', fontWeight: 500, color: '#F5F0E8', lineHeight: 1.4, margin: '12px 0 0' }}>{video.titulo}</h3>
                      </div>
                    </Link>
                  ))}
                </div>
