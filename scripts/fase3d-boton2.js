...
// Simbolo de peso sin escribir el caracter directamente en el script
...
const nuevoBtn = `              {/* Editar comision (normal / VIP) */}
              <button onClick={editarComision} disabled={procesando}
                ...
                Comisión: {inf.tipo_comision === 'monto_fijo' ? '${S}' + inf.comision_valor + '/pza' : (inf.comision_valor ?? 5) + '%'}
              </button>
...
if (ok) {
  ...
  console.log('OK: boton de comision agregado (version a prueba de simbolo peso).')
} else {
