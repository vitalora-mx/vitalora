      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '60px', flexWrap: 'wrap', gap: '24px' }}>
          ...
          <div style={{ display: 'flex', gap: '4px', background: 'var(--bg-cream)', padding: '6px', borderRadius: '100px', border: '1px solid var(--line)' }}>
            {['Todos', 'Cosméticos', 'Suplementos'].map((tab, i) => (
              <button key={tab} style={{ padding: '10px 24px', fontSize: '12px', letterSpacing: '0.15em', textTransform: 'uppercase', border: 'none', borderRadius: '100px', fontWeight: 500, cursor: 'pointer', background: i === 0 ? 'var(--black)' : 'none', color: i === 0 ? 'var(--bg-cream)' : 'var(--text)', fontFamily: 'inherit' }}>
                {tab}
              </button>
            ))}
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '32px' }}>
          {products.map((product) => (
            ...
            >
              <div style={{ aspectRatio: '4/5', background: 'var(--bg-cream)', borderRadius: '2px', overflow: 'hidden', position: 'relative', marginBottom: '20px' }}>
                <div style={{ ...tagStyles[product.tagType], position: 'absolute', top: '16px', left: '16px', padding: '6px 12px', fontSize: '10px', letterSpacing: '0.2em', textTransform: 'uppercase', zIndex: 2 }}>
                  {product.tag}
                </div>
                ...
                <button
                  ...
                  style={{
                    ...
                    padding: '14px', background: 'var(--black)', color: 'var(--bg-cream)',
              ...
              <div style={{ fontSize: '10px', letterSpacing: '0.25em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '8px' }}>{product.meta}</div>
              <h3 style={{ fontFamily: 'var(--font-cormorant), serif', fontSize: '22px', fontWeight: 500, marginBottom: '4px', color: 'var(--black)' }}>{product.nombre}</h3>
              <p style={{ fontSize: '13px', color: 'var(--muted)', marginBottom: '12px' }}>{product.sub}</p>
