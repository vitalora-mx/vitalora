    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontFamily: 'var(--font-italiana), serif', fontSize: '22px', letterSpacing: '0.1em' }}>
              Tu Carrito
            </div>
          ...
          <button
            onClick={cerrarCarrito}
            style={{ background: 'none', border: 'none', color: 'var(--bg-cream)', cursor: 'pointer', fontSize: '24px', opacity: 0.7 }}
          >✕</button>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {items.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--text-muted)' }}>
              ...
              <button
                onClick={cerrarCarrito}
                style={{ padding: '12px 24px', background: 'var(--black)', color: 'var(--bg-cream)', border: 'none', cursor: 'pointer', fontSize: '12px', letterSpacing: '0.2em', textTransform: 'uppercase', fontFamily: 'inherit', borderRadius: '2px' }}
              >
                Seguir comprando
              </button>
            </div>
            ...
            items.map((item) => (
              <div key={item.id} style={{ display: 'flex', gap: '16px', padding: '16px', background: 'white', borderRadius: '4px', border: '1px solid var(--line)' }}>
                ...
                {/* Imagen placeholder */}
                <div style={{ width: '80px', height: '80px', background: 'var(--bg-cream-deep)', borderRadius: '4px', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-italiana), serif', fontSize: '24px', color: 'var(--text-muted)' }}>
                  V
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', justifyContent: 'space-between' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', border: '1px solid var(--line)', borderRadius: '2px' }}>
                    <button
                      ...
                      style={{ width: '32px', height: '32px', border: 'none', background: 'none', cursor: 'pointer', fontSize: '16px', color: 'var(--text)' }}
                    >−</button>
        ...
        {items.length > 0 && (
          <div style={{ padding: '24px', borderTop: '1px solid var(--line)', background: 'white' }}>
            ...
            {total() >= 1000 ? (
              ...
              <div style={{ marginBottom: '16px', padding: '10px 16px', background: 'var(--bg-cream-deep)', borderRadius: '4px' }}>
                <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '6px' }}>
                  Te faltan <strong style={{ color: 'var(--black)' }}>${(1000 - total()).toLocaleString()} MXN</strong> para envío gratis
                </div>
                <div style={{ height: '4px', background: 'var(--line)', borderRadius: '2px', overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${Math.min((total() / 1000) * 100, 100)}%`, background: 'var(--gold)', borderRadius: '2px', transition: 'width 0.3s' }} />
                </div>
              </div>
            )}
            ...
            <button
              onClick={cerrarCarrito}
              style={{ width: '100%', padding: '12px', background: 'none', border: '1px solid var(--line)', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '12px', letterSpacing: '0.15em', textTransform: 'uppercase', fontFamily: 'inherit', borderRadius: '2px' }}
            >
              Seguir comprando
            </button>
          </div>
